...
public class UnBanDiscordCommand extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        ...
        if (player.isOp() || hasBypassPermission(player)) {
            event.reply("Player `" + playerName + "` cannot be unBanned.").setEphemeral(true).queue();
            return;
        }

        String command = String.format("unban %s",
                playerName);

        event.reply("Processing the unBan command...").setEphemeral(true).queue();

        Bukkit.getScheduler().runTask(KingsButBad.pl, () -> {
            boolean success = Bukkit.dispatchCommand(Bukkit.getConsoleSender(), command);
            if (success) {
                Bukkit.getScheduler().runTask(KingsButBad.pl, () -> event.getHook().editOriginal("Player `" + playerName + "` has been Unbanned").queue());
            } else {
                Bukkit.getScheduler().runTask(KingsButBad.pl, () -> event.getHook().editOriginal("Failed to execute the unbanned command. Please check the command syntax.").queue());
            }
        });
    }
