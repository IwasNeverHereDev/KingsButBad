...
public class MuteDiscordCommand extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        ...
        if (player.isOp() || hasBypassPermission(player)) {
            event.reply("Player `" + playerName + "` cannot be muted.").setEphemeral(true).queue();
            return;
        }

        String command = String.format("mute %s %s %s (Muted by %s)",
                playerName,
                time,
                reason,
                event.getMember().getEffectiveName());

        event.reply("Processing the mute command...").setEphemeral(true).queue();

        Bukkit.getScheduler().runTask(KingsButBad.pl, () -> {
            boolean success = Bukkit.dispatchCommand(Bukkit.getConsoleSender(), command);
            if (success) {
                Bukkit.getScheduler().runTask(KingsButBad.pl, () -> event.getHook().editOriginal("Player `" + playerName + "` has been muted for `" + time + "` with reason: `" + reason + "`.").queue());
            } else {
                Bukkit.getScheduler().runTask(KingsButBad.pl, () -> event.getHook().editOriginal("Failed to execute the mute command. Please check the command syntax.").queue());
            }
        });
    }
