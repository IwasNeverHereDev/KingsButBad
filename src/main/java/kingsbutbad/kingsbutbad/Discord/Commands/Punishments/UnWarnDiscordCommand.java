...
public class UnWarnDiscordCommand extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        ...
        if (player.isOp() || hasBypassPermission(player)) {
            event.reply("Player `" + playerName + "` cannot be unWarn.").setEphemeral(true).queue();
            return;
        }

        String command = String.format("unwarn %s",
                playerName);

        event.reply("Processing the unWarn command...").setEphemeral(true).queue();

        Bukkit.getScheduler().runTask(KingsButBad.pl, () -> {
            boolean success = Bukkit.dispatchCommand(Bukkit.getConsoleSender(), command);
            if (success) {
                Bukkit.getScheduler().runTask(KingsButBad.pl, () -> event.getHook().editOriginal("Player `" + playerName + "` has been unWarned.").queue());
            } else {
                Bukkit.getScheduler().runTask(KingsButBad.pl, () -> event.getHook().editOriginal("Failed to execute the unWarn command. Please check the command syntax.").queue());
            }
        });
    }
